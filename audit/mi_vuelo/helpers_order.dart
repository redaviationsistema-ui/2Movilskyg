...
class Audit {
  ...
  List<int> ids(List<Map<String, dynamic>> items) =>
      _groupedChecklist({
        'items': items,
      }).expand((g) => g.value).map((i) => i['id'] as int).toList();
}
...
void main() {
  ...
  for (final entry
      in {
        'service': ['Faltantes registrados', 'Catering sobrante registrado'],
        'operation': ['Ruta revisada', 'Aeronave revisada'],
        'cabin': [
          'Limpieza de cabina',
    ...
    if (jsonEncode(before) != jsonEncode(after) ||
        jsonEncode(before) != jsonEncode(refresh)) {
      throw StateError('Changed order');
    }
    final changedPayload = audit.ids(refreshed.reversed.toList());
    if (jsonEncode(changedPayload) != jsonEncode(before.reversed.toList())) {
      throw StateError('Did not follow payload');
    }
  }
}
